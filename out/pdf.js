...
exports.PDF = exports.STYLES = void 0;
...
var PDF = /** @class */ (function () {
    ...
    PDF.prototype.addPage = function (justNum) {
        if (justNum === void 0) { justNum = false; }
        if (!justNum) {
            this.doc.addPage("a4", "p");
        }
        this.insertText(this.doc.getNumberOfPages().toString(), [220, 610], 100)[1]();
    };
    PDF.prototype.computePadding = function (config) {
        ...
        return [
            (_c = ((_b = ((_a = config.pl) !== null && _a !== void 0 ? _a : config.px)) !== null && _b !== void 0 ? _b : config.p)) !== null && _c !== void 0 ? _c : 0,
            (_f = ((_e = ((_d = config.pt) !== null && _d !== void 0 ? _d : config.py)) !== null && _e !== void 0 ? _e : config.p)) !== null && _f !== void 0 ? _f : 0,
            (_j = ((_h = ((_g = config.pr) !== null && _g !== void 0 ? _g : config.px)) !== null && _h !== void 0 ? _h : config.p)) !== null && _j !== void 0 ? _j : 0,
            (_m = ((_l = ((_k = config.pb) !== null && _k !== void 0 ? _k : config.py)) !== null && _l !== void 0 ? _l : config.p)) !== null && _m !== void 0 ? _m : 0
        ];
    };
    PDF.prototype.addParagraph = function (text, config) {
        var _a;
        var px = (_a = config === null || config === void 0 ? void 0 : config.px) !== null && _a !== void 0 ? _a : 0;
        var width = 384 - px * 2;
        var _b = this.insertText(text, [32 + px, this.y], width, config), h = _b[0], doIt = _b[1];
        if (this.y + h > 600) {
            this.addPage();
            this.y = 32;
            var newInsert = this.insertText(text, [32 + px, this.y], width, config);
            h = newInsert[0];
            doIt = newInsert[1];
        }
        doIt();
    ...
    PDF.prototype.addRichText = function (text, baseConfig) {
        var textElements = this.richTextLayout(text, [32, this.y], 384, baseConfig);
        var pageOffset = 0;
        for (var _i = 0, textElements_1 = textElements; _i < textElements_1.length; _i++) {
            var i = textElements_1[_i];
    ...
    PDF.prototype.insertBar = function (value, maxValue, topLeft, width, config) {
        ...
        return [8 + pt + pb, function () {
                ...
                var barColor = getColor(normalizedValue);
                _this.doc.setFillColor("#DDDDDD");
                _this.doc.roundedRect(topLeft[0] + pl, topLeft[1] + pt, width - pl - pr, 8, 4, 4, "F");
                _this.doc.setFillColor(barColor);
                _this.doc.roundedRect(topLeft[0] + pl, topLeft[1] + pt, (width - pl - pr) * normalizedValue / 100, 8, 4, 4, "F");
            }];
    };
    PDF.prototype.richTextLayout = function (text, topLeft, width, baseConfig) {
        var _a;
        this.doc.setFontSize((_a = baseConfig === null || baseConfig === void 0 ? void 0 : baseConfig.size) !== null && _a !== void 0 ? _a : 11);
        var offset = [topLeft[0], topLeft[1]];
        var elements = [];
        while (text.length > 0) {
            var currentText = typeof text[0] == "string" ? text[0] : text[0].text;
            var firstLine = this.doc.splitTextToSize(currentText, width - offset[0])[0];
            var textWidth = this.doc.getStringUnitWidth(currentText) * this.doc.getFontSize() * 0.75;
            elements.push({
                text: firstLine,
                topLeft: __spreadArray([], offset, true),
                width: textWidth + 20,
                height: this.doc.getFontSize(),
                config: __assign(__assign({}, baseConfig), (typeof text[0] == "string" ? {} : text[0]))
            });
            if (currentText[0] == "\n") {
    ...
    PDF.prototype.insertText = function (text, topLeft, width, config) {
        var _this = this;
        var _a, _b, _c, _d;
        console.log(config);
        console.log(topLeft);
        var textSize = (_a = config === null || config === void 0 ? void 0 : config.size) !== null && _a !== void 0 ? _a : 11;
        this.doc.setFontSize(textSize);
        var textStyle = (_b = config === null || config === void 0 ? void 0 : config.style) !== null && _b !== void 0 ? _b : "normal";
        this.doc.setFont("Helvetica", textStyle);
        var textColor = (_c = config === null || config === void 0 ? void 0 : config.color) !== null && _c !== void 0 ? _c : "#000000";
        this.doc.setTextColor(textColor);
        var _e = this.computePadding(config), pl = _e[0], pt = _e[1], pr = _e[2], pb = _e[3];
        var splitText = this.doc.splitTextToSize(text, width - pl - pr);
        var x;
        switch ((_d = config === null || config === void 0 ? void 0 : config.align) !== null && _d !== void 0 ? _d : "left") {
            case "left":
                x = topLeft[0] + pl;
                break;
            case "center":
        ...
        return [splitText.length * textSize + pt + pb, function () {
                var _a;
                _this.doc.text(splitText, x, topLeft[1] + pt, { align: (_a = config === null || config === void 0 ? void 0 : config.align) !== null && _a !== void 0 ? _a : "left", baseline: "top" });
            }];
    };
    PDF.prototype.addTable = function (contents, widths, config) {
        var _this = this;
        // Cumlative sum starting with 0
        function cumSum0(a, base) {
            if (a.length == 0) {
                return [base !== null && base !== void 0 ? base : 0];
            }
            var prev = cumSum0(a.slice(0, -1), base);
            prev.push(a.slice(-1)[0] + prev.slice(-1)[0]);
            return prev;
        }
        var lefts = cumSum0(widths, 32);
        for (var _i = 0, contents_1 = contents; _i < contents_1.length; _i++) {
            var i = contents_1[_i];
            var h = Math.max.apply(Math, i.map(function (j, n) { return j([lefts[n], _this.y, widths[n]])[0]; }));
            if (this.y + h > 600) {
                this.y = 32;
                this.addPage();
            }
            i.map(function (j, n) { return j([lefts[n], _this.y, widths[n]])[1](); });
            this.y += h;
        }
    };
    PDF.prototype.save = function (name) {
        name = name !== null && name !== void 0 ? name : "test";
