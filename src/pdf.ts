...
export async function prepareImage(image: string): Promise<PreparedImage> {
  ...
  // Create an image element to calculate aspect ratio
  const aspectRatio: number = await new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
...
export class PDF {
  ...
  addPage() {
    if (
      this.doc.getCurrentPageInfo().pageNumber == this.doc.getNumberOfPages()
    ) {
      this.doc.addPage("a4", "p");
      this.addNum();
      if (this.headerImage) {
        this.doc.addImage(this.headerImage.image, "png", 24, 0, 32*this.headerImage.aspectRatio, 32)
      }
    } else {
  ...
  addRichText(text: RichText, baseConfig: TextConfig & PaddingConfig): void {
    ...
    for (let i of textElements) {
      ...
      this.insertText(
        i.text,
        [i.topLeft[0], i.topLeft[1] - pageOffset],
        i.width,
        i.config
      )[1]();
    ...
    return [
      ...
      () => {
        ...
        this.doc.roundedRect(
          topLeft[0] + pl,
          topLeft[1] + pt,
          ((width - pl - pr) * normalizedValue) / 100,
          8,
          4,
          4,
          "F"
  ...
  richTextLayout(
    ...
    while (text.length > 0) {
      ...
      let firstLine = this.doc.splitTextToSize(
        currentText,
        width - offset[0]
      )[0];

      let textWidth =
        this.doc.getStringUnitWidth(currentText) * fontSize * 0.75 + pl + pr;
  ...
  insertText(
    ...
    topLeft: [number, number],
    width: number,
    config?: TextConfig & PaddingConfig
  ): [number, () => void] {
    console.log(config);
    console.log(topLeft);
    ...
    return [
      splitText.length * textSize + pt + pb,
      () => {
        if (config?.bg) {
          ...
          this.doc.rect(
            ...
            topLeft[1] + pt,
            width - pl - pr - 20,
            splitText.length * textSize,
            "F"
          );
        }
        this.doc.text(splitText, x, topLeft[1] + pt, {
  ...
  addTable(
    contents: Array<
      Array<(positioning: [number, number, number]) => [number, () => void]>
    >,
  ...
  addBlock(
    f: (positioning: [number, number, number]) => [number, () => void],
    config?: BlockConfig
  ) {
    let [pl, pt, pr, pb] = this.computePadding(config);
    ...
    const [h, init] = f([
      32 + pl + borderWidth,
      this.y + pt + borderWidth,
      384 - pl - pr - borderWidth * 2,
    ]);
